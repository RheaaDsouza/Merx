...
<head>
    ...
    <link rel="stylesheet" href="css/styleha.css">
...
<body>
  ...
  <nav class="navbar navbar-expand-sm bg-dark navbar-dark fixed-top" id="mainNav">
    <div class="container">
      ...
      <li><button type="button" class="btn btn-dark" data-toggle="modal" data-target="#cartModal"><i class="fa fa-cart-arrow-down fa-lg"></i></button></li>
      </ul>
    </div>
  </nav>
  ...
  <header class="mainheader">
    <div class="container">
      <h1 id="heading">Home Appliances</h1>
    </div>
  </header>
  ...
  <a id="btnEmpty" href="indexHomeApp.php?action=empty">Empty Cart</a>
  ...
  <tr>
    ...
    <td style="text-align:center;"><a href="indexHomeApp.php?action=remove&code=<?php echo $item["code"]; ?>" class="btnRemoveAction"><i class="fa fa-trash" style="color:black;" aria-hidden="true"></i></a></td>
    </tr>
    <?php
...
<div id="product-grid">
  ...
  <div class="row">
  <?php
  $product= mysqli_query($con,"SELECT * FROM products WHERE category='Home' ORDER BY id ASC");
  if (!empty($product)) { 
    while ($row=mysqli_fetch_array($product)) {
    
  ?>
  
    <form method="post" action="indexHomeApp.php?action=add&pid=<?php echo $row["id"]; ?>">
    <div class="card" style="width:18rem">
      <img class="card-img-top"  style="height:18rem;" src="<?php echo $row["image"]; ?>" alt="High Speed Blender">
      <div class="card-body">
      <h5 class="card-title"><?php echo $row["name"]; ?></h5>
      <p class="card-text"><?php echo "₹".$row["price"]; ?></p>
      <div class="cart-action">Quantity<input type="text" class="product-quantity" name="quantity" value="1" size="2" /><input type="submit" value="Add to Cart" class="btnAddAction" /></div>
    </div>
    </div>
    </form>
